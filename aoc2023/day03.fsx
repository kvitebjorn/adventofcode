...
let toCoord idx length = (idx % length, idx / length)
...
let solvePt1 (rawInput: string) =
    ...
    let isValidNum (num, idx) =
        let numIndices =
            seq {
                for x in idx .. (idx + String.length (num) - 1) do
                    yield x
            }
        ...
        let distances =
            Seq.map (fun num -> Seq.map (fun x -> isAdjacent num x) symbolCoords) numCoords

        let validNums =
            Seq.filter (fun d -> Seq.length (Seq.filter (fun x -> x = true) d) > 0) distances
...
let solvePt2 (rawInput: string) =
    ...
    let gearToNums =
        Seq.map (fun x -> Seq.filter (fun y -> isValidNum y x) numbers) gearCoords

    let validGears = Seq.filter (fun x -> Seq.length x = 2) gearToNums

    let gearRatios =
        Seq.map (fun nums -> Seq.fold (*) 1 (Seq.map (fun num -> int (fst num)) nums)) validGears
