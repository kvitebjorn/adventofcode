...
let compareEntries (n1: list<char>, s1: int) (n2: list<char>, s2: int) = compareHands n1 n2
...
let counts s =
    List.filter (fun c -> c <> 'J') s
    |> List.distinct
    |> List.map (fun x -> List.filter (fun y -> y = x) s |> List.length)
    |> List.sortDescending

let jokers s =
    List.filter (fun c -> c = 'J') s |> List.length
...
let compareEntriesPt2 (n1: list<char>, s1: int) (n2: list<char>, s2: int) = compareHandsPt2 n1 n2
