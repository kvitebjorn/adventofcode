...
let parsed = File.ReadAllLines "input.txt" 
            |> Array.toList
            |> List.map Seq.toList
            |> parse []
let rotate n list = 
...
let rec tryFindSymmetry l symmetryFn col =
    ...
    if isInBounds then
        ...
        | true -> col
        | _    -> tryFindSymmetry l symmetryFn (col + 1) else 0
let reflection l symmetryFn = 
    let h = tryFindSymmetry l symmetryFn 1  
    if h = 0 then tryFindSymmetry (rotate 90 l) symmetryFn 1 else h * 100
let total = List.map (fun x -> reflection x (fun l r -> l = (List.rev r))) parsed |> List.sum
printfn $"{total}"

let symmetryFnPt2 l r = 
    (l, List.rev r)
    ||> List.zip 
    |> List.map (fun (c1 : char list, c2 : char list) -> 
        List.mapi (fun i x -> if c1[i] = x then 0 else 1) c2)
    |> List.map List.sum |> List.sum |> (fun m -> m = 1)
