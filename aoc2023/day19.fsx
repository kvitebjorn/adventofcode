...
let makeFn (body: string) =
    ...
    let f =
        (fun (p: Part) ->
            let result =
                Array.map (fun f -> f (p)) expressions
...
let accepted =
    Seq.filter (fun p -> p.result.IsSome && p.result.Value = "A") parts.Values
...
let intersect (interval1: Interval) (interval2: Interval) =
    { A = max interval1.A interval2.A
      B = min interval1.B interval2.B }
...
let rec tree goto priority' depth' (xmasMap: XmasMap) =
    ...
    | "R" ->
        ...
        |> Some
    | _ ->
        if priority' = wfNameToExpressions[goto].Length then
            None
        else
            ...
            let xmasMapLeft =
                ...
                | true ->
                    xmasMap
                    |> Map.map (fun k v ->
                        if k = expr.var.Value then
                            intersect
                                v
                                (if isLessOp then
                                     { A = 1; B = expr.num.Value - 1 }
                                 else
                                     { A = expr.num.Value + 1; B = 4000 })
                        else
                            v)
                | _ -> xmasMap
                    ...
                    |> Map.map (fun k v ->
                        if k = expr.var.Value then
                            intersect
                                v
                                (if isLessOp then
                                     { A = expr.num.Value; B = 4000 }
                                 else
                                     { A = 1; B = expr.num.Value })
                        else
                            v)
                | _ -> xmasMap
